...
export const superAdminSignup = async (req: Request, res: Response) =>{
    try{
        ...
        const registered_superAdmin = await superadmin.findOne({email: body.email});
        ...
        res.cookie("token", createToken({username: body.name, email: body.email}),{
            path: "/superadmin",
            httpOnly: true,
            encode: btoa,
...
export const superAdminLogin = async (req: Request, res: Response) =>{
    try{
        ...
        const isValidPassword = await superAdmin_data.isValidPassword(body.password);
        ...
        res.cookie("token", createToken({username: superAdmin_data.username, email: superAdmin_data.email}),{
            path: "/superadmin",
            httpOnly: true,
            encode: btoa,
            expires: new Date(new Date().setDate(new Date().getDate() + 30 ))
